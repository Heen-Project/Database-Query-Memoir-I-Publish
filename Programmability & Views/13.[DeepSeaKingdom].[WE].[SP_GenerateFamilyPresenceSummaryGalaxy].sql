USE [DeepSeaKingdom]
GO
CREATE PROCEDURE [WE].[SP_GenerateFamilyPresenceSummaryGalaxy] (
	@Term NVARCHAR(MAX) = N''
)
...
SET @LinkedQuery = 'IF OBJECT_ID(''tempdb.dbo.#TempBusinessPresences'', ''U'') IS NOT NULL DROP TABLE #TempBusinessPresences;
	...
	SELECT DISTINCT [WorkshopName] = lab.Name
		...
		,[TotalPresent] = (CASE WHEN COUNT(ctd.BrandBusinessDetailId) OVER (PARTITION BY ct.BrandBusinessId,bFamilyXXX.MemberId) = COUNT(ta.BusinessPresenceId) OVER (PARTITION BY ct.BrandBusinessId, ta.MemberId) THEN CONVERT(VARCHAR, ISNULL(SUM(CASE WHEN ta.Status LIKE ''Present##%'' OR ta.Status LIKE ''Dispensation##%'' THEN 1 ELSE NULL END) OVER (PARTITION BY ct.BrandBusinessId, ta.MemberId),0)) ELSE ''N/A'' END)
		,[TotalAbsent] = (CASE WHEN COUNT(ctd.BrandBusinessDetailId) OVER (PARTITION BY ct.BrandBusinessId,bFamilyXXX.MemberId) = COUNT(ta.BusinessPresenceId) OVER (PARTITION BY ct.BrandBusinessId, ta.MemberId) THEN CONVERT(VARCHAR, ISNULL(SUM(CASE WHEN ta.Status LIKE ''Late##%'' OR ta.Status LIKE ''Absent##%'' THEN 1 ELSE NULL END) OVER (PARTITION BY ct.BrandBusinessId, ta.MemberId),0)) ELSE ''N/A'' END)
