USE [DeepSeaKingdom]
GO
CREATE FUNCTION [WE].[UDFTV_CheckMarkComplainFamily] (@QuarterSession VARCHAR(MAX))
...
OPEN CheckMarkComplain_Cursor
	...
	BEGIN 
		...
		SELECT @CheckMarkComplainId, @QuarterSessionId, @WorkshopId, @FormNumber, @ComplainNumber, @CheckerId, @CheckerBy, @Description, @SavedBy, @SavedDate, as_Familys.Type, as_Familys.Number, as_Familys.MemberId, as_Familys.OldMark, as_Familys.OldStatus, as_Familys.NewMark, as_Familys.NewStatus, as_Familys.TopicId
		FROM (SELECT DISTINCT * FROM OPENJSON(@Familys) WITH (Type nvarchar(max),Number int,MemberId nvarchar(max),OldMark int,OldStatus nvarchar(max), NewMark int, NewStatus nvarchar(max), TopicId nvarchar(max))) as_Familys
		--
	FETCH NEXT FROM CheckMarkComplain_Cursor INTO @CheckMarkComplainId, @QuarterSessionId, @WorkshopId, @FormNumber, @ComplainNumber, @CheckerId, @CheckerBy, @Description, @SavedBy, @SavedDate, @Familys--, @SummaryFamilys
	END  
